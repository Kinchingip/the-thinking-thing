// headtracking-sketch.js — face-mesh visualization for Ending B
// Adapted from the standalone headtracking.js sketch.
// Load order: this file first, then ml5.js, then p5.js.
// p5 global mode picks up window.setup / window.draw automatically.

let gameMode = true; // suppresses standalone UI chrome (stamps, keyboard hints)
...
let showCameraTrash = gameMode; // always show camera feed when running in-game
let showMesh = true;
let showType = true;
let showNoise = true;
let showBands = true;
...
function setup() {
  ...
  cnv.id('face-riot-canvas');
  cnv.parent('ending-b-terminal');
  cnv.style('position', 'fixed');
  cnv.style('top', '0');
  cnv.style('left', '0');
  cnv.style('z-index', '40');
  ...
  try {
    ...
    // MediaPipe unavailable — face tracking disabled, sketch still runs
  }
...
// Called by MediaPipe with each processed frame.
// Converts normalized [0,1] coords to video-pixel coords so updateTracking() is unchanged.
function gotFaces(results) {
  if (results && results.multiFaceLandmarks && results.multiFaceLandmarks.length > 0) {
    const vw = (cam.elt && cam.elt.videoWidth) || baseW;
    const vh = (cam.elt && cam.elt.videoHeight) || baseH;
    faces = [{
      keypoints: results.multiFaceLandmarks[0].map(lm => ({
        x: lm.x * vw,
        y: lm.y * vh,
        z: (lm.z || 0) * vw
      }))
...
function draw() {
  ...
  if (!gameMode) drawTopStamp(p);
  if (!gameMode) drawBottomStamp(p);
  drawBorderRiot(p);
  drawGlobalGlitch(p);
}
...
function drawNoFacePoster(p) {
  ...
  fill(colorAlpha(p.accent, 130));
  textSize(18);
  text('SCANNING FOR SUBSTRATE…', width / 2, height / 2 - 16);
  fill(colorAlpha(p.fg, 70));
  textSize(11);
  text('BIOLOGICAL SIGNATURE REQUIRED', width / 2, height / 2 + 14);
  pop();
}
